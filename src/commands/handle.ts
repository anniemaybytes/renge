...
export function listenForStaffHandle() {
  IRCClient.addMessageHookInChannel(IRCClient.staffSupportChan, handleMatchRegex, async (event) => {
    ...
    if (matches[1] && matches[2]) {
      ...
      try {
        await SupportSessionManager.startSupportSession(matches[1], event.nick, false, matches[2], 'N/A');
        // Remove user from queue if they were in the queue, ignoring errors
        SupportQueue.unqueueUser(undefined, matches[1]).catch(() => '');
      } catch (e) {
    ...
    try {
      ...
      return await SupportSessionManager.startSupportSession(user.nick, event.nick, true, user.reason, user.ip);
    } catch (e) {
      return event.reply(`${e}`);
    }
  });
